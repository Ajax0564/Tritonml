...
class FlashAttentionCausal(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, mask,offset=0, sm_scale=None, causal=True):
        if sm_scale is None:
...
class TritonCausalAttention(torch.nn.Module):
    def __init__(self, sm_scale=None,causal=True):
        super().__init__()
    ...
    def forward(self, q, k, v, mask,start_pos=0):
        ...
        return FlashAttentionCausal.apply(q, k, v, mask,start_pos, scale,self.causal)